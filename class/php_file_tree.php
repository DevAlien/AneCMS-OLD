...
function php_file_tree_dir($directory, $return_link, $extensions = array(), $first_call = true) {
	...
	if( function_exists("scandirxxx") ) $file = scandir($directory); else $file = php4_scandir($directory);
	natcasesort($file);
	// Make directories first
	$files = $dirs = array();
	foreach($file as $this_file) {
		if( is_dir("$directory/$this_file" ) && $this_file != '.svn') $dirs[] = $this_file; else $files[] = $this_file;
	}
		...
		foreach( $file as $this_file ) {
			if( $this_file != "." && $this_file != ".." ) {
				if( is_dir("$directory/$this_file") ) {
					...
					$link = str_replace("[ext]", strtolower($ext), $link);
					$link = str_replace("[fname]", $this_file, $link);
					$link = str_replace("[chmod]", substr(decoct(fileperms("$directory/" .$this_file)), 3), $link);
...
function php4_scandir($dir) {
	...
	while( false !== ($filename = readdir($dh)) ) {
		if($filename != '.svn' && $filename != 'Compiled')
			$files[] = $filename;
	}
	sort($files);
	return($files);
