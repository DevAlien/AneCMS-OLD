...
if($user->isOnGroup('Administrator') && isset($_POST['json']) && $_POST['json'] != ''){
    $idtheme = $db->query_array('SELECT id,widgetarea FROM '.$database['tbl_prefix'].'dev_themes WHERE name = \''.$skin.'\'');
    ...
    foreach($aWidgets as $widgetarea => $aWithWidgets){
        $c = 0;
            ...
            echo 'INSERT INTO '.$database['tbl_prefix'].'dev_themewidgets (idtheme, widgetname, widgetarea, position) VALUES ('.$idtheme['id'].', \''.$aWithWidget.'\', \''.$widgetarea.'\', '.$c.')';
            }
    }
